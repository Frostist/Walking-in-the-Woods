...
import { WaveManager, WaveConfig } from './WaveManager.js';
...
// Load game configs
let enemyConfigs: Map<string, EnemyConfig>;
...
try {
    ...
    enemyConfigs = new Map<string, EnemyConfig>();
    Object.entries(enemiesData).forEach(([key, value]) => {
        enemyConfigs.set(key, value as EnemyConfig);
    });

    // Load waves config
    const wavesPath = join(__dirname, '../config/waves.json');
    waveConfig = JSON.parse(readFileSync(wavesPath, 'utf-8')) as WaveConfig;

    // Load spawns config
    const spawnsPath = join(__dirname, '../config/spawns.json');
    spawnPoints = JSON.parse(readFileSync(spawnsPath, 'utf-8')) as SpawnPoint[];
...
let currentGameState: string = 'Boot';
...
io.on('connection', (socket) => {
    ...
    socket.on('enemyDamaged', (data: { enemyId: string; damage: number; element?: string }) => {
    ...
    socket.on('startWave', (data: { waveIndex: number }) => {
