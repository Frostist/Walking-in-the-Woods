...
import { PlayerState } from './MonsterManager.js';
...
export class EnemyManager {
  ...
  private fireProjectile(enemy: EnemyState, target: PlayerState, config: EnemyConfig): void {
  ...
  private findNearestPlayer(enemyPos: { x: number; y: number; z: number }): PlayerState | null {
  ...
  public spawnEnemy(type: string, position: { x: number; y: number; z: number }): string {
  ...
  public getEnemyState(enemyId: string): EnemyState | undefined {
    return this.enemies.get(enemyId);
  }

  public getAllEnemies(): EnemyState[] {
